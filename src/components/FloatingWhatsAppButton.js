...
export function FloatingWhatsAppButton(){
  const phone = '256708188110'
  const href = `https://wa.me/${phone}`
  return (
    <a href={href} target="_blank" rel="noreferrer" className="fixed right-4 bottom-6 z-50 group" aria-label="Chat with us on WhatsApp">
      <div className="relative bg-green-500 hover:bg-green-600 text-white p-3 rounded-full shadow-lg flex items-center justify-center transition-transform duration-300 group-hover:-translate-y-0.5">
        <span className="absolute inset-0 rounded-full bg-green-400/40 animate-ping" aria-hidden="true" />
        <span className="absolute -inset-2 rounded-full border-2 border-green-400/50 animate-pulse" aria-hidden="true" />
